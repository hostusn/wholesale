...
if ( ! function_exists( 'helendo_comment' ) ) :
	function helendo_comment( $comment, $args, $depth ) {
		...
		echo helendo_comment_template( $comments );
	}
...
if ( ! function_exists( 'helendo_comment_template' ) ) :
	function helendo_comment_template( $comments ) {
		...
		$date      = sprintf( esc_html__( '%1$s at %2$s', 'helendo' ), $comments['comment_date'], $comments['comment_time'] );
		$output[]  = sprintf( '<a href="%s" class="date">%s</a>', esc_url( $comments['comment_link'] ), $date );
		$output[]  = '</div>';
		if ( $comments['comment_approved'] == '0' ) {
			$output[] = sprintf( '<em class="comment-awaiting-moderation">%s</em>', esc_html__( 'Your comment is awaiting moderation.', 'helendo' ) );
		} else {
		...
		if ( current_user_can( 'edit_comment', $comments['comment_ID'] ) ) {
			$output[] = sprintf( '<a class="comment-edit-link" href="%s">%s</a>', esc_url( admin_url( 'comment.php?action=editcomment&amp;c=' ) . $comments['comment_ID'] ), esc_html__( 'Edit', 'helendo' ) );
		}
