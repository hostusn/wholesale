...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
        <thead>
        <tr>
            <th class="product-name"><?php esc_html_e( 'Name', 'helendo' ); ?></th>
            <th class="product-quantity"><?php esc_html_e( 'Qty', 'helendo' ); ?></th>
        </tr>
        </thead>
        <tbody>
			<?php
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						case 'label':
							...
							$value .= $grouped_product_child->is_visible() ? '<a class="product-title" href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', $grouped_product_child->get_permalink(), $grouped_product_child->get_id() ) ) . '">' . $grouped_product_child->get_name() . '</a>' : $grouped_product_child->get_name();
							$value .= '</label>';
