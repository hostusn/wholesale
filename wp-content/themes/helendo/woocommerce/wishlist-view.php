            ...
            <div class="wishlist-title <?php echo esc_attr( $custom_list_class ); ?>">
				...
				<?php if ( $is_custom_list ): ?>
                    <a class="btn button show-title-form">
						...
						<?php esc_html_e( 'Edit title', 'helendo' ) ?>
                    </a>
				<?php endif; ?>
            </div>
			<?php if ( $is_custom_list ): ?>
            ...
            <tr>
				<?php if ( $show_cb ) : ?>
                    ...
                    <th class="product-checkbox">
                        <input type="checkbox" name="" id="bulk_add_to_cart"/>
                    </th>
                ...
                <th class="product-name">
                    <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product', 'helendo' ) ) ?></span>
                </th>
				...
				<?php if ( $show_price ) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'helendo' ) ) ?>
                    </span>
                    </th>
				...
				<?php if ( $show_stock_status ) : ?>
                    ...
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'helendo' ) ) ?>
                    </span>
                    </th>
			...
			if ( count( $wishlist_items ) > 0 ) :
                                ...
                                <td class="product-remove">
                                    <div>
                                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>"
                                           ...
                                           title="<?php esc_attr_e( 'Remove this product', 'helendo' ) ?>">&times;</a>
                                    </div>
                                </td>
							<?php endif; ?>
                            ...
                            <td class="product-name">
                                ...
                                <p class="m-price hidden-lg hidden-md">
									...
									if ( $base_product ) {
										...
										echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'helendo' ) );
									}
									?>
                                </p>
                                ...
                                <p class="m-stock-status hidden-lg hidden-md">
									...
									if ( $stock_status == 'out-of-stock' ) {
										echo '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'helendo' ) . '</span>';
									} else {
										echo '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'helendo' ) . '</span>';
									} ?>
                                </p>
                            </td>
							...
							<?php if ( $show_last_column ): ?>
                                <td class="product-add-to-cart hidden-xs">
									...
									if ( $show_dateadded && isset( $item['dateadded'] ) ):
										echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'helendo' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
									endif;
									?>
									...
									<?php if ( $available_multi_wishlist && is_user_logged_in() && count( $users_wishlists ) > 1 && $move_to_another_wishlist && $is_user_owner ): ?>
                                        <select class="change-wishlist selectBox">
                                            <option value=""><?php esc_html_e( 'Move', 'helendo' ) ?></option>
											<?php
											foreach ( $users_wishlists as $wl ):
                                                ...
                                                <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
													...
													if ( $wl['wishlist_privacy'] == 1 ) {
														$wl_privacy = esc_html__( 'Shared', 'helendo' );
													} elseif ( $wl['wishlist_privacy'] == 2 ) {
														$wl_privacy = esc_html__( 'Private', 'helendo' );
													} else {
														$wl_privacy = esc_html__( 'Public', 'helendo' );
													}
									...
									<?php if ( $is_user_owner && $repeat_remove_button ): ?>
                                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>"
                                           ...
                                           title="<?php esc_attr_e( 'Remove this product', 'helendo' ) ?>"><?php esc_html_e( 'Remove', 'helendo' ) ?></a>
									<?php endif; ?>
                                </td>
							<?php endif; ?>
                        </tr>
                ...
                <tr>
                    <td colspan="<?php echo esc_attr( $column_count ) ?>"
                        class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products were added to the wishlist', 'helendo' ) ) ?></td>
                </tr>
			<?php
			endif;
					...
					<?php if ( $show_cb ) : ?>
                        <div class="custom-add-to-cart-button-cotaniner">
                            <a href="<?php echo esc_url( add_query_arg( array(
                               ...
                               id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'helendo' ) ) ?></a>
                        </div>
					<?php endif; ?>
					<?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
                        <div class="ask-an-estimate-button-container">
                            <a href="<?php if ( $additional_info || ! is_user_logged_in() ) { echo '#ask_an_estimate_popup'; } else { echo esc_url( $ask_estimate_url ); } ?>"
                               class="btn button ask-an-estimate-button" <?php if( $additional_info ) { echo 'data-rel="prettyPhoto[ask_an_estimate]"'; } ?> >
								<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', '<i class="fa fa-shopping-cart"></i>' ) ?>
								<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'helendo' ) ) ?>
                            </a>
                        </div>
					<?php endif; ?>
    ...
    <div id="ask_an_estimate_popup">
        <form action="<?php echo esc_url( $ask_estimate_url ); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if ( ! is_user_logged_in() ): ?>
                <label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'helendo' ) ) ?></label>
